...
interface LocaleBlock { tr?: any[]; ru?: any[]; en?: any[]; uz?: any[] }

export default function BenefitsGrid({ faydalar, currentLang }: { faydalar: { baslik?: LocaleString; aciklama?: LocaleBlock; gorselUrls?: string[]; videolar?: string[]; videoDosyaUrls?: string[] }[]; currentLang?: string }) {
  const [openIdx, setOpenIdx] = useState<number | null>(null);
  // Sadece dolu öğeleri göster ve aynı listeyi modal için kullan (index kayması olmasın)
  const items = (faydalar || []).filter((f) => !!f && (
    f.baslik || 
